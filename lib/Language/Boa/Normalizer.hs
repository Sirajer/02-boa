...
--------------------------------------------------------------------------------
imm i (Number n l)      = (i, [], ImmExpr n)--error "TBD:imm:Number"

imm i (Id x l)          = (i, [], Expr x)--error "TBD:imm:Id"
...
imm i (Prim2 o e1 e2 l) = (i''', b1s ++ b2s ++ [(t, Prim2 o v1 v2 i''')], Id t) --error "TBD"
  where
    (t, i')             = fresh l i
    (i'', b1s, v1)       = imm i' e1
    (i''', b2s, v2)      = imm i'' e2
